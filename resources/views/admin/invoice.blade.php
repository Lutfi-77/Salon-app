...
<body>
    <div class="flex h-screen w-full items-center justify-center bg-gray-600">
        <div class="w-80 rounded bg-gray-50 px-6 pt-8 shadow-lg">
            ...
            <div class="flex flex-col justify-center items-center gap-2">
                <h4 class="font-semibold">Charisma Salon</h4>
            ...
            <div class="flex flex-col gap-3 pb-6 pt-2 text-xs">
                <table class="w-full text-left">
                    <thead>
                        <tr class="flex">
                            <th class="w-full py-2">Treatment</th>
                            <th class="min-w-[44px] py-2">Price</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($transaction->appointment->detail as $detailAppointment)
                        <tr class="flex">
                            <td class="flex-1 py-1">{{$detailAppointment->detailTreatment->name}}</td>
                            <td class="min-w-[44px]">{{format_rupiah($detailAppointment->price)}}</td>
                        </tr>
                        @endforeach
                        <tr class="flex">
                            <th class="flex-1 py-2">Total</th>
                            <td class="min-w-[44px] py-2">{{format_rupiah($transaction->total_price)}}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="py-4 justify-center items-center flex flex-col gap-2">
                    <p class="flex gap-2"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            ...
                            <path
                                ...
                                fill="#000"></path>
                        </svg> charisma.salon@gmail.com</p>
                    <p class="flex gap-2"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            viewBox="0 0 24 24" fill="none">
                            <path fill="#000"
    ...
    <a href="{{route('admin.transaction.index')}}" class="print:hidden absolute bottom-5 right-5 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">Back</a>
</body>
